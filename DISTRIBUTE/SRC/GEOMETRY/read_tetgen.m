function [mymesh,boundary_mat] = read_tetgen(fname,para_deform,Ncmpt,Nboundary)
...
boundary_mat = zeros(mymesh.Ncmpt,mymesh.Nboundary);
for icmpt = 1:mymesh.Ncmpt
    for ibd = 1:mymesh.Nboundary
        boundary_mat(icmpt,ibd)=~isempty(mymesh.Pts_boundary_reorder{icmpt}{ibd});
    end
end
